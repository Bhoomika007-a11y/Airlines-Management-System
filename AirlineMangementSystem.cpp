...
{
public:
	string name = "", date = "", departureCity = "", destinationCity = "", gender = "", Aadhar = "";
...
public:
	...
	{
		if (toLower(city) == "mumbai" || toLower(city) == "chennai" ||
			toLower(city) == "bangalore" || toLower(city) == "jaipur")
		{
			return true;
		}
		return false;
	...
	void inputPassengersInfo()
	{
		...
		do {
			if (change == 1 || change == 2)
			{
				...
				{
					cout << "DEPARTURE\n";
					cout << "    Our flights are only available from Chennai, Kolkata, Bangalore, Mumbai and Jaipur.\n";
			...
			if (change == 1 || change == 3)
			{
				...
				{
					cout << "DESTINATION\n";
					cout << "    Our flights are only available to Chennai, Kolkata, Bangalore, Mumbai and Jaipur.\n";
		...
		for (int i = 1; i <= members; i++)
		{
			...
			passenger_newnode->Aadhar = Aadhar;
		...
		if (destinationCity == "Mumbai" || destinationCity == "mumbai")
		{
			call.mumbai();
		}
		else if (destinationCity == "Bangalore" || destinationCity == "bangalore")
		{
			call.bangalore();
		}
	...
	void confirmPayment()
	{
		...
		cout << "\nEnter Aadhar number of booking of 1st person: ";
		cin >> Aadhar;
		...
		while (current != NULL)
		{
			if (bcode == current->bookingCode && Aadhar == current->Aadhar)
			{
				found = true;
				break;
			}
